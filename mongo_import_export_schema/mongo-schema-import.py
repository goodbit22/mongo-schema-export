...
def mongo_import(
    ...
    with open(fname, "r") as input_conf:
        ...
        for dbname, d in conf["databases"].items():
            if (
                databases is not None
                and databases != "*"
                and dbname not in databases.split(",")
            ):
...
def main(argv=sys.argv):
    ...
    parser.add_argument(
        "--password", metavar="pwd", type=str, help="Username", default=""
    )
    parser.add_argument(
        "--authSource",
        metavar="a",
    ...
    parser.add_argument(
        "--delete-db", action="store_true", help="Delete database if it exists"
    )
    parser.add_argument(
        "--delete-col", action="store_true", help="Delete collections if they exist"
    )
    ...
    parser.add_argument(
        ...
        metavar="db",
        type=str,
        help="Select databases from the config json to insert, default is all of them",
        default="*",
    )
